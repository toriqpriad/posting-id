...
if (!isset($_COOKIE['csrf_cookie_pid'])) {
    exit('ERROR 2');
}

if (!isset($_COOKIE['fm_url'])) {
    exit('ERROR 3');
}

session_start();
$defaults = array(
    CURLOPT_URL  => $_COOKIE['fm_url'],
    CURLOPT_POST => true,
);
$ch     = curl_init();
$docurl = curl_setopt_array($ch, ($defaults));
if (!$docurl) {
    exit('ERROR 4');
} else {
    $_SESSION["upload_dir"] = $_COOKIE['fm_dir'];
}
